...
const ActionColumn = ({ row }) => {
    const INIT_TEMAS = {
        id_foro: row.cells[1].value ? row.cells[1].value : row.cells[1].value,
    ...
    const eliminar = () => {
        ...
        swal({
            ...
            if (d) {
                const urla = `https://aulaweb.autoevaluacion.com.co/v2/?&accion=editorforos&${response}&opcion=delete&operacion=sindoc`;
    ...
    const actualizar = (event) => {
        ...
        if (validated) {
            ...
            const urla = `https://aulaweb.autoevaluacion.com.co/v2/?&accion=editorforos&${response}&opcion=editar&operacion=sindoc`;
            const respuesta = api.createactividad(`${urla}`);
            respuesta.then(function (resp) {
                const records = resp;
                if (records) {
            ...
            if (temas.selectedFile) {
                ...
                const queryDatos = datosfiles
                    ? Object.keys(datosfiles)
                          ...
                          .join('&')
                    : '';
                const urla = `https://aulaweb.autoevaluacion.com.co/v2/?&accion=editorforos&id_autor=${props.state.id_pernatural}&grupo_id=${props.state.id_grupo}&cod_materia=${props.state.codmateria}&id_curso=${props.state.id_curso}&objeto=Foro&${queryString}&${queryDatos}&opcion=registrar&operacion=doc&autor=Docente`;
                ...
                const urla = `https://aulaweb.autoevaluacion.com.co/v2/?&accion=editorforos&id_autor=${props.state.id_pernatural}&grupo_id=${props.state.id_grupo}&cod_materia=${props.state.codmateria}&id_curso=${props.state.id_curso}&objeto=Foro&${queryString}&opcion=registrar&operacion=sindoc&autor=Docente`;
                const respuesta = api.createactividad(`${urla}`);
                respuesta.then(function (resp) {
                    const records = resp;
                    if (records) {
        ...
        <Card>
            <Card.Body>
                <Button variant="danger" className="btn-rounded mb-3" onClick={toggleSignUp}>
                    <i className="mdi mdi-plus"></i> Crear Foro
                </Button>
...
const EditorForos = (props: foros): React$Element<any> => {
    const [records, openForos] = useState([]);
    const [data, cargarForos] = useState([]);
    const [datafiles, setFiles] = useState([]);
    useEffect(() => {
        const urla = `https://aulaweb.autoevaluacion.com.co/v2/?&accion=editorforos&id_autor=${props.state.id_pernatural}&grupo_id=${props.state.id_grupo}&cod_materia=${props.state.codmateria}&id_tema=${props.state.id_tema}&id_curso=${props.state.id_curso}&opcion=consultar&operacion=sindoc`;
                    ...
                    <Card>
                        <Card.Body>
                            <h4 className="header-title">
                                Foro: {props.state.id_curso}/{props.state.codmateria}-{props.state.nombrecurso}-
